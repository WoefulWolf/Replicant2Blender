...
def master_rs_layer4(textures_dir: str, material: bpy.types.Material, instance: tpGxMaterialInstanceV2):
    ...
    # Mapping 4
    mapping_4 = nodes.new(type='ShaderNodeMapping')
    mapping_4.location = grid_location(-1, 0)
    mapping_4.hide = True
    ...
    links.new(mapping_4.outputs['Vector'], tex_layer_mask.inputs['Vector'])
    ...
    mapping_3.location = grid_location(-1, 9)
    mapping_3.hide = True
    ...
    mix_col_0123.location = grid_location(4, 3)
    mix_col_0123.hide = True
    links.new(tex_layer_mask_sep.outputs[2], mix_col_0123.inputs['Fac'])
    ...
    mix_orm_01.location = grid_location(2, 5)
    ...
    mix_orm_012.location = grid_location(3, 6)
    ...
    # Mix ORM 0-1-2-3
    mix_orm_0123 = nodes.new(type='ShaderNodeMixRGB')
    mix_orm_0123.location = grid_location(4, 7)
    mix_orm_0123.hide = True
    links.new(tex_layer_mask_sep.outputs[2], mix_orm_0123.inputs['Fac'])
    links.new(mix_orm_012.outputs['Color'], mix_orm_0123.inputs['Color1'])
    links.new(tex_orm_3.outputs['Color'], mix_orm_0123.inputs['Color2'])

    # ORM 0-1-2-3 Separate Color
    orm_0123_sep = nodes.new(type=sepRGB_name)
    orm_0123_sep.location = grid_location(5, 7)
    orm_0123_sep.hide = True
    links.new(mix_orm_0123.outputs['Color'], orm_0123_sep.inputs[sepRGB_input])

    # ORM 0-1-2-3 multiply AO
    ao_multiply = nodes.new('ShaderNodeMixRGB')
    ao_multiply.location = grid_location(6, 3)
    ...
    mix_normal_01.location = grid_location(2, 9)
    ...
    mix_normal_012.location = grid_location(3, 10)
    ...
    mix_normal_0123.location = grid_location(4, 11)
    mix_normal_0123.hide = True
    links.new(tex_layer_mask_sep.outputs[2], mix_normal_0123.inputs['Fac'])
    links.new(mix_normal_012.outputs['Color'], mix_normal_0123.inputs['Color1'])
    links.new(tex_normal_3.outputs['Color'], mix_normal_0123.inputs['Color2'])

    # Convert DirectX normal to OpenGL
    normal_dx_gl = nodes.new('ShaderNodeGroup')
    normal_dx_gl.node_tree = dx_to_gl_normal()
    normal_dx_gl.location = grid_location(5, 11)
    normal_dx_gl.hide = True
    links.new(mix_normal_0123.outputs['Color'], normal_dx_gl.inputs['Color'])

    # Normal Map
    normal_map = nodes.new(type='ShaderNodeNormalMap')
    normal_map.location = grid_location(6, 11)
    normal_map.hide = True
    links.new(normal_dx_gl.outputs[0], normal_map.inputs['Color'])

    # Principled BSDF
    principled = nodes.new(type='ShaderNodeBsdfPrincipled')
    principled.location = grid_location(7, 4)
    links.new(ao_multiply.outputs['Color'], principled.inputs['Base Color'])
    links.new(orm_0123_sep.outputs[1], principled.inputs['Roughness'])
    links.new(orm_0123_sep.outputs[2], principled.inputs['Metallic'])
    links.new(normal_map.outputs['Normal'], principled.inputs['Normal'])

    # Output
    output = nodes.new(type='ShaderNodeOutputMaterial')
    output.location = grid_location(8, 4)
