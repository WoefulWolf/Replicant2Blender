...
def master_rs_layer2(textures_dir: str, material: bpy.types.Material, instance: tpGxMaterialInstanceV2):
    # Renamed in 5.0
    ...
    mapping_2.location = grid_location(-1, 0)
    mapping_2.hide = True
    links.new(uv1.outputs['UV'], mapping_2.inputs['Vector'])

    # gUVOffset2
    g_uv_offset_2 = constant_buffer_value(material, nodes, instance, "CbLayer2", "gUVOffset2")
    if g_uv_offset_2 is not None:
        g_uv_offset_2.location = grid_location(-2, 0)
        links.new(g_uv_offset_2.outputs[0], mapping_2.inputs['Location'])

    # gUVScale2
    g_uv_scale_2 = constant_buffer_value(material, nodes, instance, "CbLayer2", "gUVScale2")
    if g_uv_scale_2 is not None:
        g_uv_scale_2.location = grid_location(-2, 1)
    ...
    links.new(mapping_2.outputs['Vector'], tex_layer_mask.inputs['Vector'])
    ...
    # gUVOffset0
    g_uv_offset_0 = constant_buffer_value(material, nodes, instance, "CbLayer2", "gUVOffset0")
    ...
    # gUVScale0
    g_uv_scale_0 = constant_buffer_value(material, nodes, instance, "CbLayer2", "gUVScale0")
    ...
    # gUVOffset1
    g_uv_offset_1 = constant_buffer_value(material, nodes, instance, "CbLayer2", "gUVOffset1")
    ...
    # gUVScale1
    g_uv_scale_1 = constant_buffer_value(material, nodes, instance, "CbLayer2", "gUVScale1")
    ...
    tex_orm_0.location = grid_location(0, 3)
    ...
    # ORM 0-1 Separate Color
    orm_01_sep = nodes.new(type=sepRGB_name)
    orm_01_sep.location = grid_location(3, 3)
    orm_01_sep.hide = True
    links.new(mix_orm_01.outputs['Color'], orm_01_sep.inputs[sepRGB_input])

    # ORM 0-1 multiply AO
    ao_multiply = nodes.new('ShaderNodeMixRGB')
    ao_multiply.location = grid_location(4, 1)
    ao_multiply.hide = True
    ao_multiply.blend_type = 'MULTIPLY'
    ao_multiply.inputs[0].default_value = 1.0
    links.new(mix_col_01.outputs['Color'], ao_multiply.inputs[1])
    links.new(orm_01_sep.outputs[0], ao_multiply.inputs[2])
    ...
    tex_normal_0.location = grid_location(0, 5)
    ...
    normal_dx_gl.location = grid_location(3, 5)
    normal_dx_gl.hide = True
    links.new(mix_normal_01.outputs['Color'], normal_dx_gl.inputs['Color'])

    # Normal Map
    normal_map = nodes.new(type='ShaderNodeNormalMap')
    normal_map.location = grid_location(4, 5)
    normal_map.hide = True
    links.new(normal_dx_gl.outputs[0], normal_map.inputs['Color'])

    # Principled BSDF
    principled = nodes.new(type='ShaderNodeBsdfPrincipled')
    principled.location = grid_location(5, 1)
    links.new(ao_multiply.outputs['Color'], principled.inputs['Base Color'])
    links.new(orm_01_sep.outputs[1], principled.inputs['Roughness'])
    links.new(orm_01_sep.outputs[2], principled.inputs['Metallic'])
    links.new(normal_map.outputs['Normal'], principled.inputs['Normal'])

    # Output
    output = nodes.new(type='ShaderNodeOutputMaterial')
    output.location = grid_location(6, 1)
